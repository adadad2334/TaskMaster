...
def create_task(db: Session, task: schemas.TaskCreate):
    task_data = task.dict(exclude={"required_skills"})
...
def update_task(db: Session, task_id: int, task: schemas.TaskUpdate):
    ...
    update_data = task.dict(exclude={"required_skills"}, exclude_unset=True)
    for key, value in update_data.items():
    ...
    if "assignee_id" in update_data:
        if db_task.assignee_id:  # Если был предыдущий исполнитель
            ...
            if prev_assignee:
                prev_assignee.current_workload -= db_task.estimated_hours
                if prev_assignee.current_workload < 0:
                    prev_assignee.current_workload = 0
...
def delete_task(db: Session, task_id: int):
    ...
    if db_task:
        ...
        if db_task.assignee_id:
            ...
            if assignee:
                assignee.current_workload -= db_task.estimated_hours
                if assignee.current_workload < 0:
                    assignee.current_workload = 0
